...
export default function TaskModal() {
  ...
  const [hasPresetProject, setHasPresetProject] = useState(false);

  useEffect(() => {
    const d = modal?.defaults || {};
    setHasPresetProject(!!d.projectId);
    if (isEdit && task) {
      setForm({
  ...
  const handleSubmit = (e) => {
    ...
    else { addTask(data); showToast('משימה נוספה בהצלחה'); }
    handleClose();
  };
          ...
          <div className="px-6 py-5 space-y-4">
            ...
            {!isEdit && (
              <button
                ...
                className="text-sm text-indigo-600 hover:text-indigo-800 font-medium py-0.5 transition-colors"
              >
                {showExpanded ? '− פחות פרטים' : '+ פרטים נוספים'}
              </button>
            )}

            {/* ── EXPANDED FIELDS ── */}
            <div className={`expandable-grid${showExpanded ? ' open' : ''}`}>
              <div className="expandable-inner">
                <div className="space-y-4">

                  {/* Project / Topic — shown here when NOT opened from a project */}
                  {!hasPresetProject && (form.taskType === 'project' || form.taskType === 'recurring') && (
                    <div>
              ...
              </div>{/* end expandable-inner */}
            </div>{/* end expandable-grid */}
